...
- 主窗口提供归档清单；任务行尾可一键归档，归档后从“待办”和“已完成”中隐藏，在“归档”清单中可撤回恢复。
...
- 可与 Obsidian Markdown 清单双向同步；在设置中选择任意 `.md` 文件即可切换当前展示的待办清单。
- Obsidian 中保存 `- [ ]` / `- [x]` 格式待办后，桌面组件会自动读取并更新；组件内新增、完成、删除和子任务变更也会写回 Markdown。
- 解锁状态下点击任务文字可直接在原行编辑标题，回车或点到别处保存，Esc 取消；选中文字时使用主题融合的灰色选区。
...
设置中的“Obsidian 联动”默认关闭。启用后点击“选择文件”，选择一个 Markdown 清单文件即可同步。文件中使用 Obsidian 标准任务格式：
...
组件会在任务行尾添加 `<!--qtw:...-->` 隐藏 ID，用于在 Obsidian 中改标题后仍能识别同一个任务。写回时会替换文件中的任务清单区域，并保留任务区之前和之后的正文内容。

## 数据位置

- 设置：`%APPDATA%\QuietTodoWidget\settings.json`
- 本地任务：`%APPDATA%\QuietTodoWidget\local_tasks.json`
- Obsidian 同步文件：由用户在设置中选择；默认建议位置为当前用户的 `Documents\QuietTodoWidget\tasks.md`
